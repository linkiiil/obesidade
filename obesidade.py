...
def main():
    ...
    st.markdown(
        "**Origem dos dados:** Os dados referem-se a campanhas de saúde e estilo de vida e foram usados, com expansão via SMOTE, para treinar este sistema."
    )

    # Aviso médico (importante)
    st.warning("Este sistema não substitui avaliação médica ou nutricional. Consulte um profissional de saúde para diagnóstico e recomendações personalizadas.")

    tab_pred, tab_viz = st.tabs(["🔍 Descobrir Meu Perfil", "📊 Painel Técnico de Clusters"])

    with tab_pred:
        # NOTA DE ORIGEM DOS DADOS (detalhe)
        st.info(
            "**Sobre a Inteligência deste Sistema:** Este modelo não usa apenas dados reais. Para ser mais preciso, ele foi 'ensinado' com um dataset expandido artificialmente (via técnica SMOTE), o que permite que ele entenda perfis que estão entre as categorias comuns."
        )
        ...
        with st.form("formulario_obesidade_final"):
            ...
            with c1:
                gender = st.selectbox("Gênero", [1.0, 0.0], format_func=lambda x: "Feminino" if x == 1.0 else "Masculino")
                ...
                fam_hist = st.selectbox("Histórico familiar de sobrepeso?", [1.0, 0.0], format_func=lambda x: "Sim" if x == 1.0 else "Não")

            with c2:
                favc = st.selectbox("Consome alimentos calóricos frequentemente?", [1.0, 0.0], format_func=lambda x: "Sim" if x == 1.0 else "Não")
                ...
                smoke = st.selectbox("É fumante?", [1.0, 0.0], format_func=lambda x: "Sim" if x == 1.0 else "Não")

            with c3:
                ch2o = st.slider("Consumo de água (Litros/dia)", 1.0, 3.0, 2.0, step=0.1)
                scc = st.selectbox("Monitoriza o consumo de calorias?", [1.0, 0.0], format_func=lambda x: "Sim" if x == 1.0 else "Não")
    ...
    # Rodapé com aviso médico reforçado
    st.divider()
    st.caption("Este sistema não substitui avaliação médica ou nutricional. Consulte um profissional de saúde para diagnóstico e recomendações personalizadas.")
